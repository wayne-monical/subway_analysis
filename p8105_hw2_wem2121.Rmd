...
Here we reading in the 2024 Mr Trash Wheel sheet using the `real_excel` function. We have specified the excel sheet and the data range, and we have rounded the number of sports balls and set it to integer values.  
```{r}
trash = 
  read_excel(
    ...
    range = 'A2:N653') |> 
  janitor::clean_names() |> 
  mutate(sports_balls = sports_balls |> round() |> as.integer())
```
